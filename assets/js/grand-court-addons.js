...
(function(){
  ...
  function dollars(n){ return Math.round(Number(n||0) * 100) / 100; }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderAttendees(){
      ...
      list.querySelectorAll('button[data-del]').forEach(btn=>{
        btn.onclick = ()=>{ Cart.removeAttendee(btn.dataset.del); renderAttendees(); refreshAttSelects(); };
      });
    }
    form.onsubmit = (e)=>{
      e.preventDefault();
    ...
    function renderAddonCard(addon){
      ...
      if (addon.type === 'fixed') {
        ...
        controls = `
          <div class="row">
            ...
            <label>Amount (USD)
              <input type="number" min="${addon.minAmount||0.01}" step="0.01" value="${addon.minAmount||0}" inputmode="decimal" class="amt">
            </label>
        ...
        const options = (addon.variants||[]).map(v=>`<option value="${v}">${v}</option>`).join('');
        controls = `
          <div class="row">
            <label>Assign to
              <select class="att-select"></select>
      ...
      const priceLabel = addon.type === 'amount'
        ? 'Custom Amount'
        : money(addon.price || 0);
    ...
    function renderAddons(){
      grid.innerHTML = window.GRAND_COURT_ADDONS.map(renderAddonCard).join('');
      refreshAttSelects();
      ...
      grid.querySelectorAll('.act-add').forEach(btn=>{
        btn.addEventListener('click', (e)=>{
          ...
          if (!attendeeId){ alert('Please select an attendee first.'); return; }

          // Enforce limit per attendee for "fixed"
          if (addon.type === 'fixed' && addon.limitPerAttendee){
            const st = Cart.get();
            const already = (st.lines||[]).some(l => l.attendeeId===attendeeId && l.itemType==='addon' && l.itemId===addon.id);
            if (already){ alert(`${addon.name} already added for this attendee.`); return; }
          }
          ...
          if (addon.type === 'fixed'){
            ...
            if (!isFinite(amt) || amt < min){ alert(`Enter an amount ≥ ${min}.`); return; }
            Cart.addLine({
              attendeeId,
              itemType: 'addon',
              itemId: addon.id,
            ...
            const qty     = Math.max(1, parseInt(card.querySelector('.qty').value||'1',10));
            const name    = custom && variant==='Custom' ? `${addon.name} (Custom)` : `${addon.name} (${variant})`;
            const notes   = custom || '';
            const itemId  = variant==='Custom'
              ? `${addon.id}:custom`
